...
- **Click without the mouse.** Press `F` (Shift+F) and every link, button and field in view gets a label; type a label to click it (or to type into the field). `g f` opens the link in a new tab instead. Esc closes the labels, and the characters they use can be changed in the settings. Plain `f` stays with the site, so it still means fullscreen on YouTube.
