...
Imports flow one way: entrypoints, then dom / storage / components, then core. Core imports from no other layer.
...
- In-page UI is vanilla TypeScript + plain CSS in px units, rendered in the one shared shadow root, with text set through `textContent`. React and Tailwind belong to the popup and options page.
