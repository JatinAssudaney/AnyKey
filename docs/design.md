...
- Such tabs are marked on the toolbar button, so people see the problem before they press a key that does nothing (`src/background/badge.ts`). On install and update the background asks every open tab's content script `ping`, which answers only that AnyKey runs, never the page's address, and gives each tab that doesn't answer a `!` badge and a tooltip that says to reload. The browser keeps a tab's badge and tooltip with the tab and resets them when it loads a page, which is also what starts AnyKey there, so nothing needs to clear the mark. A page that was loading when it was asked can start AnyKey just after, so the marked tabs are asked again a second later and unmarked if they answer. Browser pages get the mark too, since the background can't tell them apart without their addresses; it goes as soon as they load another page. No permission is needed.
