...
- The popup can't read the tab's URL without the `tabs` permission, so it asks the tab's content script (`pageInfo`). No answer means AnyKey isn't running there: a browser page, or a web page AnyKey couldn't start in (see below), which "Reload this tab" fixes.
- The browser adds the content script only to pages that load after AnyKey is installed or updated, so on install and update the background starts AnyKey in the tabs already open (`src/background/openTabs.ts`), into the page as it stands. Reloading them instead would lose what people typed, and an update arrives while they work. It asks each tab's content script `ping`, which answers only that AnyKey runs, never the page's address, and runs the content script file (`scripting.executeScript`) in each tab that doesn't answer. After an update the tab's old copy has dead APIs and lets every key through, and a second copy of the same version stops the first (WXT's `ContentScriptContext`). Tabs the browser discarded to save memory are left alone: they load again when opened, and AnyKey with them. Browser pages, the Web Store and AnyKey's own pages refuse the script. This needs `scripting` and host access to every site, which the content script has already, so the install warning is the same.
